...
module.exports = {
    ...
    checkout :  async(req, res) => {
        try {
            ...
            if (!res_voucher)  res.status(404).json({message : `voucher tidak ditemukan`})

            const res_nominal =  await Nominal.findOne({ _id : nominal })
            if (!res_nominal)  res.status(404).json({message : `nominal tidak ditemukan`})
